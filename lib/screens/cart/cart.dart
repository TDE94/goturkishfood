  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer(builder: (context, watch, child) {
        // Carttaki ürün miktarını dinleyip ona göre fonksiyon çalıştırır
        final provider = watch(cartChangeNotifierProvider);
        var price = provider.getTotalprice();

        if(provider.cartList.length == 0 ){
          return Center(child: Text("No Product, Go to Store"),);
        }else 
